...
export const createSurat = async (req: Request, res: Response) => {
  try {
    ...
    if (!Object.values(JenisSurat).includes(jenis_surat)) {
      return res
        .status(400)
        .json({ success: false, message: "Jenis surat tidak valid!" });
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json({ success: false, message: "User warga tidak ditemukan!" });
    }

    const alasanKeperluan =
    ...
    if (!alasanKeperluan) {
      return res
        ...
        .json({
          success: false,
          message: "Alasan keperluan surat wajib diisi!",
        });
    }
    ...
    if (!alamatDb || !rtDb || !rwDb) {
      return res.status(400).json({
        success: false,
        message:
          "Data domisili (alamat, RT, RW) belum lengkap di profil. Silakan lengkapi profil warga terlebih dahulu.",
      });
    }
    ...
    return res
      ...
      .json({
        success: false,
        message: error.message || "Gagal membuat pengajuan surat",
      });
  }
};
...
export const getSuratByWarga = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Gagal memuat riwayat surat" });
  }
};
...
export const getAllSuratAdmin = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Gagal memuat antrean surat admin" });
  }
};
...
export const verifySuratAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (!Number.isFinite(id)) {
      return res
        .status(400)
        .json({ success: false, message: "ID surat tidak valid" });
    }
    ...
    if (!surat) {
      return res
        ...
        .json({
          success: false,
          message: "Berkas pengajuan surat tidak ditemukan!",
        });
    }

    if (status === StatusSurat.SELESAI) {
      // 🧠 ENGINE OTOMATISASI GENERATOR NOMOR SURAT RESMI DESA DIGITAL
      const totalSelesai = await suratRepository.countBy({
      ...
      if (!alasan_ditolak) {
        return res
          ...
          .json({
            success: false,
            message: "Alasan penolakan surat wajib diisi!",
          });
      }
    ...
    return res
      ...
      .json({
        success: false,
        message: "Gagal memproses eksekusi berkas surat",
      });
  }
};
