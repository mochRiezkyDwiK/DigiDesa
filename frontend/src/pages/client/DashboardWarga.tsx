  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const suratRes = await axios.get("http://localhost:5000/api/v1/surat/riwayat", { headers }).catch(() => null);
        
        // Ambil data pengaduan dengan fallback rute alternatif
        const laporanRes = await axios.get("http://localhost:5000/api/v1/pengaduan/riwayat", { headers })
          .catch(async () => {
            return await axios.get("http://localhost:5000/api/v1/user/pengaduan/riwayat", { headers }).catch(() => null);
          });
      ...
      <aside className="hidden lg:flex w-72 bg-white border-r border-slate-100 flex-col sticky top-0 h-screen z-40 shadow-sm flex-shrink-0">
        ...
        <nav className="flex-1 px-6 space-y-1.5 mt-4">
          ...
          {[
            ...
            { n: "Ajukan E-Surat", i: FileText, p: "/buat-surat", isTab: false },
                ...
                <section className="relative p-10 bg-slate-900 rounded-[3rem] overflow-hidden shadow-[0_20px_50px_-15px_rgba(15,23,42,0.3)]">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    ...
                    <div className="flex gap-4">
                      <motion.button onClick={() => navigate('/buat-surat')} whileHover={{ y: -5 }} className="px-8 py-4 bg-blue-600 text-white font-black rounded-2xl text-sm shadow-xl shadow-blue-500/25 hover:bg-blue-500 transition-all flex items-center gap-3">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid gap-5">
                      {suratList.length === 0 ? (
                        ...
                        suratList.map((surat) => (
                          <Link 
                            to={"/layanan/detail/" + surat.id} 
                            key={surat.id} 
                            className="block no-underline transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
                            title="Klik untuk melihat detail pengajuan"
                          >
                            <div className="p-7 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm group hover:border-blue-500 hover:shadow-md transition-all">
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                                <div className="flex items-center gap-5">
                                  ...
                                  <div>
                                    <h4 className="font-black text-slate-800 text-base tracking-tight group-hover:text-blue-600 transition-colors">
                                ...
                                <div className="flex items-center gap-8">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <ChevronRight size={16} className="text-slate-300 group-hover:text-blue-500 group-hover:translate-x-1 transition-all" strokeWidth={3} />
                                  </div>
                                </div>
                              </div>
                            </div>
